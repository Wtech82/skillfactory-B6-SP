...
def add_album(year, artist, genre, album):
    '''
    Добавляет нового артиста в текущюю БД
    '''
    ...
    session = connect_db()
    new_album = session.query(Album).filter(Album.album == album, Album.artist == artist).first()
    if new_album is not None:
        raise AlreadyExists("Album already exists and has #{}".format(new_album.id))
